...
%% read data from file
salmon_populations=readtable('dungeness_2005-2020_combine_ma_msd.csv');

%% select species to focus on (we chose 4, can add more)
species_per_hour=[table2array(salmon_populations(:,5:6)),table2array(salmon_populations(:,13:14))]';%time-series data
%We chose chinook0_hatchery_perhour,chinook0_wild_perhour,coho1_hatchery_perhour,coho1_wild_perhour 
...
p=zeros(4,size(J,3));
p(1,:)=J(1,2,:);%chinook0 wild to chinook0 hatchery
p(2,:)=J(2,1,:);%chinook0 hatchery to chinook0 wild
